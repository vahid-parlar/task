...
export class UserProfileComponent {
  ...
  profileForm = this.builder.group({
    ...
    mobile: this.builder.control(this.data.mobile, Validators.required),
  });
  ...
  changeProfileInfo() {
    if (this.profileForm.valid && this._mobileValidator()) {
      ...
      this.userService
        ...
        .pipe(
          take(1),
          map((res) => {
            const login = this.localStorageService.getLoginResult() as LoginResult;
  ...
  private _mobileValidator() {
    const phoneRegex = /^09\d{9}$/;
    const isValidPhone = phoneRegex.test(
      this.profileForm.value.mobile as string
    );
